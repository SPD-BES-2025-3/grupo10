...
interface User {
  ...
  // Adicione outros campos que seu usuário possa ter, como: username, phone, etc.
}
...
export default function UsersPage() {
  ...
  useEffect(() => {
    ...
    fetchUsers(); // Chamamos a função para iniciar a busca.
    ...
    <main className="bg-slate-900 min-h-screen p-8 text-white flex justify-center">
      <div className="w-full max-w-4xl">
        ...
        <div className="bg-slate-800 p-6 rounded-lg shadow-lg">
          <h2 className="text-2xl font-semibold mb-4">Lista de Usuários do Backend</h2>

          {/* Renderização Condicional */}
          {loading && <p className="text-yellow-400">Carregando usuários...</p>}
          ...
          {!loading && !error && (
            <ul className="space-y-4">
              {users.map(user => (
                <li key={user._id} className="bg-slate-700 p-4 rounded-md flex justify-between items-center">
                  <div>
